...
def show_key_expansion():
    """Menampilkan proses Key Expansion AES secara detail dengan visualisasi yang fixed"""
    st.subheader("🔑 Key Expansion AES")
    
    # S-Box AES
    sbox = [
        0x63, 0x7c, 0x77, 0x7b, 0xf2, 0x6b, 0x6f, 0xc5, 0x30, 0x01, 0x67, 0x2b, 0xfe, 0xd7, 0xab, 0x76,
        # ... (isi lengkap S-Box seperti sebelumnya)
    ]
    ...
    # Membagi menjadi 4 word (32-bit each)
    ...
    # Simulasi perhitungan untuk 44 word (AES-128)
    ...
    rounds = []
    for i in range(11):  # 10 round + kunci awal
        if i == 0:
            round_name = "Kunci Awal"
            round_bytes = w0 + w1 + w2 + w3
        else:
            round_name = f"Round {i}"
            start = i*4
            round_bytes = (expanded_keys[start] + expanded_keys[start+1] + 
                         expanded_keys[start+2] + expanded_keys[start+3])
        
        rounds.append([round_name] + [f"{b:02X}" for b in round_bytes])
    
    # Kolom untuk DataFrame
    columns = ["Round"] + [f"Byte {i}" for i in range(16)]
    
    # Tampilkan tabel dengan st.dataframe untuk fleksibilitas lebih
    st.dataframe(
        pd.DataFrame(rounds, columns=columns),
        height=(11 * 35) + 38,  # Tinggi disesuaikan
        use_container_width=True,
        hide_index=True
    )
    
    # Alternatif visualisasi dengan CSS
    st.markdown("""
    <style>
    .key-schedule {
    ...
    .key-schedule th, .key-schedule td {
    ...
    .key-schedule tr:nth-child(even) {
        background-color: #f9f9f9;
    }
    </style>
    """, unsafe_allow_html=True)
    
    # Buat HTML table manual untuk kontrol lebih baik
    html_table = ["<div class='key-schedule'><table>"]
    html_table.append("<tr><th>Round</th>" + "".join(f"<th>Byte {i}</th>" for i in range(16)) + "</tr>")
    
    for round_data in rounds:
        html_table.append("<tr>" + "".join(f"<td>{cell}</td>" for cell in round_data) + "</tr>")
...
def main():
    st.title("AES Key Expansion Visualizer")
    show_key_expansion()
